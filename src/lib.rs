...
static NATIVE_DISPLAY: RwLock<Option<Arc<Mutex<native::NativeDisplayData>>>> = RwLock::new(None);
fn set_display(display: native::NativeDisplayData) {
    let mut lock = NATIVE_DISPLAY
        .write()
        .expect("Failed to acquire write lock");
    if lock.is_some() {
        panic!("NATIVE_DISPLAY already set");
    }
    *lock = Some(Arc::new(Mutex::new(display)));
}
fn clear_display() {
    let mut lock = NATIVE_DISPLAY
        .write()
        .expect("Failed to acquire write lock");
    *lock = None; // Drop the display, allowing a new one to be set
}
fn native_display() -> Arc<Mutex<native::NativeDisplayData>> {
    let lock = NATIVE_DISPLAY.read().expect("Failed to acquire read lock");
    lock.as_ref()
